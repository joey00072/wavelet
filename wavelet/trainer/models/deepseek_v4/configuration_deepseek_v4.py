"""DeepSeek-V4 configuration, adapted from Apache-2.0 PrimeRL."""
...
from .compat import DEEPSEEK_V4_LAYER_TYPES, allow_deepseek_v4_layer_types
...
class DeepseekV4Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if layer_types is None and legacy_compress_ratios is not None:
            ratio_to_layer_type = {
                0: "sliding_attention",
                **{rate: lt for lt, rate in self.compress_rates.items()},
            }
            layer_types = [
                ratio_to_layer_type[ratio]
                for ratio in legacy_compress_ratios[:num_hidden_layers]
            ]
        ...
        self.compress_ratios = [
            self.compress_rates.get(lt, 0) for lt in self.layer_types
        ]
        ...
        self.mlp_layer_types = ["hash_moe"] * min(
            num_hidden_layers, num_hash_layers
        ) + ["moe"] * max(0, num_hidden_layers - num_hash_layers)
        self.rope_parameters = self._nest_rope_parameters(raw_rope_parameters)
        self.validate_architecture()

    def _nest_rope_parameters(
        self, raw_rope_parameters: dict[str, Any] | None
    ...
    def validate_architecture(self) -> None:
        ...
        for layer_type in set(self.layer_types) - {"sliding_attention"}:
            if layer_type not in self.compress_rates:
                raise ValueError(
                    f"compress_rates is missing a rate for layer type {layer_type!r}."
                )
    ...
    def validate_layer_type(self) -> None:
        ...
        if unknown:
            raise ValueError(
                f"layer_types entries must be one of {DEEPSEEK_V4_LAYER_TYPES}; got {unknown}."
            )
