...
def http_base_urls(config: RLConfig, count: int | None = None) -> list[str]:
    from wavelet.inference.client import http_base_urls as resolve_http_base_urls

    return resolve_http_base_urls(config, count)


def required_inference_devices(config: RLConfig) -> int:
    return config.inference.vllm.tensor_parallel_size * (
