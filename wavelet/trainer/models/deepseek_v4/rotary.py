"""DeepSeek-V4 dual rotary embeddings, adapted from Apache-2.0 PrimeRL."""
...
SUPPORTED_ROPE_TYPES = frozenset(
    {"default", "linear", "llama3", "proportional", "yarn"}
)
...
def rotate_half_interleaved(x: torch.Tensor) -> torch.Tensor:
    ...
    The GPT-NeoX layout pairs channel `i` with channel `i + dim / 2`. DeepSeek-V4 stores each
...
def apply_rotary_pos_emb_interleaved(
    ...
    rotated = ((rope.float() * cos) + (rotate_half_interleaved(rope).float() * sin)).to(
        x.dtype
    )
...
class DeepseekV4RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: DeepseekV4Config, device: torch.device | None = None):
        ...
        self.layer_types = [
            name
            for name, params in config.rope_parameters.items()
        ...
        for layer_type in self.layer_types:
            ...
            inv_freq, attention_scaling = self._rope_init_fn(layer_type)(
                config, device, layer_type=layer_type
            )
    ...
    def _rope_init_fn(
        self, layer_type: str
    ) -> Callable[..., tuple[torch.Tensor, float]]:
